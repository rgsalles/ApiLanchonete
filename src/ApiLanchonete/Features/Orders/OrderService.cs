...
public class OrderService(AppDbContext context) : IOrderService
{
    ...
    public async Task<OrderDto> CreateOrder(CreateOrderDto dto)
    {
        ...
        var warehouseByProduct = await context.Warehouses
            .Where(warehouse => warehouse.BranchId == branch.Id && productIds.Contains(warehouse.ProductId))
            .ToDictionaryAsync(warehouse => warehouse.ProductId);

        foreach (var item in dto.Items)
        {
            if (!warehouseByProduct.TryGetValue(item.ProductId, out var warehouse) || !warehouse.Active)
        ...
        foreach (var productId in productIds)
        {
            ...
            var warehouse = warehouseByProduct[productId];
            var totalPrice = product.Price * quantity;

            warehouse.Quantity -= quantity;
            warehouse.UpdatedAt = now;
            warehouse.UpdatedBy = "Order";
    ...
    public async Task UpdateOrder(Guid id, UpdateOrderDto dto)
    {
        ...
        if (dto.Status == OrderStatus.Cancelled && order.StockReserved)
        {
            ...
            var warehouseByProduct = await context.Warehouses
                .Where(warehouse => warehouse.BranchId == order.BranchId && productIds.Contains(warehouse.ProductId))
                .ToDictionaryAsync(warehouse => warehouse.ProductId);

            foreach (var item in order.Items)
            {
                if (!warehouseByProduct.TryGetValue(item.ProductId, out var warehouse))
                    throw new BadRequestException("The order warehouse entry was not found.");

                warehouse.Quantity += item.Quantity;
                warehouse.UpdatedAt = DateTime.UtcNow;
                warehouse.UpdatedBy = "Order cancellation";
            }
